...
const checkInputAll = () => {
    ...
    if(linkfill){
        ...
        getData.forEach(item => {
            ...
            const existItem = newData.findIndex(newItem => newItem.title === item.title && newItem.size === item.size  && newItem.color === item.color && newItem.id === item.id);
            if(existItem !== -1){
                newData[existItem].quantity = parseInt(newData[existItem].quantity) + item.quantity;
